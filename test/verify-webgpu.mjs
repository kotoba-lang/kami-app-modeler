...
const recovered = await page.evaluate(() => { const state = JSON.parse(document.querySelector("#debug-state").textContent); return {objectCount: state.objectCount, texture: state.material["base-color-texture"], uvCount: state.uvCount}; });
if (!(recovered.objectCount === 1 && recovered.uvCount === knife.vertices && recovered.texture.startsWith("data:image/png;base64,"))) throw new Error(`Backup project recovery lost production state: ${JSON.stringify(recovered)}`);
