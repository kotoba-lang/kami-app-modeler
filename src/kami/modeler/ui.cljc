  ...
  (html/html5
   [:html {:lang "en"}
    ...
    [:body [:header [:strong "KAMI MODELER"] [:a {:href "https://kotoba-lang.github.io/kami-studio/"} "Studio"] [:span "WebGPU polygon workspace"] [:button {:id "new-cube"} "New Cube"] [:button {:id "import"} "Import EDN"] [:input {:id "import-file" :type "file" :accept ".edn" :style {:display "none"}}] [:button {:id "export"} "Export EDN"]]
     [:main [:aside [:h2 "Scene"] [:button.object.selected "◆ Cube"] [:h2 "Tools"] [:button.selected {:data-command "select"} "Select Face"] [:button {:id "inset"} "Inset"] [:button {:id "scale"} "Scale Face"] [:button {:id "move"} "Move Face"] [:button {:id "delete"} "Delete Face"] [:h2 "Interaction profile"] [:select {:id "profile"} [:option {:value "blender"} "Blender"] [:option {:value "maya"} "Maya"] [:option {:value "max"} "3ds Max"] [:option {:value "c4d"} "Cinema 4D"]] [:p.hint {:id "shortcutHint"} "Click Select · E Extrude · MMB Orbit · Ctrl+Z Undo"]]
      ...
      [:aside [:h2 "Face Inspector"] [:p {:id "selection"} "Face 1 selected"] [:label "Operation amount " [:output {:id "distanceValue"} "0.50"] [:input {:id "distance" :type "range" :min "0.05" :max "2" :step "0.05" :value "0.5"}]] [:button.primary {:id "extrude"} "Extrude selected face"] [:h2 "History"] [:ol {:id "history"} [:li "Create cube"]] [:button {:id "undo"} "Undo"] [:button {:id "redo"} "Redo"]]]
